...
fs.readFile.Sync("inde.html", "utf-8", (err, contenido) => {
    if (err){
        console.log(err);
    }else{
...
fs.rename.Sync("index.html", "main.html", (err) => {
    if (err){
        throw err;
...
/*Ejemplo para agregar contenido al final del archivo  método appendfile*/
 ...
 fs.appendFile.Sync("index.html", "<p>Hola</p>", (err) => {
    if (err){
        throw err;
    }else{/*El else no necesariamente se lo puede utilizar*/
 ...
 fs.writeFile.Sync("index.html", "Contenido Nuevo", (err) => {
    if(err){
        throw err;
    }
